...
<div id="comment-{{ $comment->getKey() }}" class="media">
    <img class="mr-3 w-100 rounded-circle" style="max-width: 40px;" src="{{ $post->user->profile->profileImage()}}">
    <div class="media-body">
        ...
        <div>
            ...
            @can('edit-comment', $comment)
                <button data-toggle="modal" data-target="#comment-modal-{{ $comment->getKey() }}" class="btn btn-sm btn-link text-lowercase">@lang('comments::comments.edit')</button>
            @endcan
            @can('delete-comment', $comment)
                <a href="{{ route('comments.destroy', $comment->getKey()) }}" onclick="event.preventDefault();document.getElementById('comment-delete-form-{{ $comment->getKey() }}').submit();" class="btn btn-sm btn-link text-danger text-lowercase">@lang('comments::comments.delete')</a>
